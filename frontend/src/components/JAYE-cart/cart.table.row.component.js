...
const TableRowItem = ({ bookId, bookName, bookPrice, quantity }) => {
   const classes = useStyles();
   const [productId, setProductId] = useState(bookId)
   const [productName, setProductName] = useState(bookName)
   let [productPrice, setProductPrice] = useState(bookPrice)
   ...
   const deleteCartItem = async () => {
      ...
      await axios.delete(`http://localhost:8059/cart/delete/${productId}`, config)
         .then(() => {
            console.log('deleted')
            window.location = '/me'
            return (
